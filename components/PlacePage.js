...
const PlacePage = {
  template:
    "<div className='header-part'><div class='top-part' ><p>Welcome to the {{ place.name }} Page!</p><router-link :to='`/locations/${place.location_id}/places`'>Go Back</router-link></div> <br> <br>       <form v-if='!message && !message.length' @submit.prevent='updatePlace()'><h2>Update the place</h2> <div style='display:flex;flex-direction: column;'><label for='name'>Place Name</label><input  name='name'  v-model='place.name' type='text' /></div><div class='lat-long'><div><label for='latitude'>Latitude</label><input  v-model='place.lat' name='lat' type='number' /></div><div><label for='latitude'>Longitude</label><input name='lng'  v-model='place.lng' type='number' /></div></div><ul class='contains-task-list'><li class='task-list-item'><label><input @click='toggleVisited()' type='checkbox' :checked='place.visited'><span v-if='!this.place.visited '>Not visited</span><span v-if='this.place.visited '>Visited</span></label></li></ul> <button class='btn-add' type='submit'>Update place</button> <button @click='deletePlace(place.id)' class='btn-delete'>Delete</button></form></div><br> <br>   <div class='empty-page' v-if='!place '>Pas de place a afficher</div><div class='message-success' v-if='message'><p>{{message}}</p> </div>",
  data() {
    return {
      place: "",
      name: "",
